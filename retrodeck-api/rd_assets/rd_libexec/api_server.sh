...
retrodeck_api() {
  case "$1" in
    start)  api_start_server::retrodeck-api ;;
    stop)   api_stop_server::retrodeck-api ;;
    status) api_status_server::retrodeck-api ;;
    *)
...
api_start_server::retrodeck-api() {
...
api_status_server::retrodeck-api() {
  if [[ -f "$api_pid_file" ]] && kill -0 "$(cat "$api_pid_file")" 2>/dev/null; then
...
api_handle_request::retrodeck-api() {
  # USAGE: api_handle_request::retrodeck-api "$json_request_line"

  local json_input="$1"

  # Validate JSON format
  if ! jq empty <<< "$json_input" 2>/dev/null; then
    api_build_response::retrodeck-api "error" "unknown" "" "Invalid JSON format"
    return
  fi
  ...
  if [[ -z "$request_id" ]]; then
    api_build_response::retrodeck-api "error" "unknown" "" "Missing required field: request_id"
    return
  fi

  if [[ -z "$api_version" ]]; then
    api_build_response::retrodeck-api "error" "$request_id" "" "Missing required field: version"
    return
  fi

  if [[ -z "$action" ]]; then
    api_build_response::retrodeck-api "error" "$request_id" "" "Missing required field: action"
    return
  fi
  ...
  case "$action" in
    "check_status")
      api_build_response::retrodeck-api "success" "$request_id" '"ok"' ""
      return
      ;;
    "list_endpoints")
      local endpoints
      endpoints=$(api_builtin_list_endpoints::retrodeck-api)
      api_build_response::retrodeck-api "success" "$request_id" "$endpoints" ""
      return
      ;;
  esac

  if [[ -z "$request" ]]; then
    api_build_response::retrodeck-api "error" "$request_id" "" "Missing required field: request"
    return
  fi

  api_dispatch_request::retrodeck-api "$action" "$request" "$request_data" "$request_id"
}

api_dispatch_request::retrodeck-api() {
  # USAGE: api_dispatch_request::retrodeck-api "$action" "$request" "$request_data" "$request_id"
  ...
  if [[ -z "$endpoint_def" ]]; then
    api_build_response::retrodeck-api "error" "$request_id" "" "Unknown endpoint: $endpoint_key"
    return
  fi
  ...
  if ! declare -f "$target_function" > /dev/null 2>&1; then
    api_build_response::retrodeck-api "error" "$request_id" "" "Endpoint function not found: $target_function"
    return
  fi
  ...
  if [[ -n "$request_data" && "$request_data" != "null" ]]; then
    if ! jq empty <<< "$request_data" 2>/dev/null; then
      api_build_response::retrodeck-api "error" "$request_id" "" "Invalid JSON in data field"
      return
    fi
  ...
  if [[ -n "$missing_fields" ]]; then
    api_build_response::retrodeck-api "error" "$request_id" "" "Missing required data fields: $missing_fields"
    return
  fi
  ...
  if result=$("$target_function" "${func_args[@]}"); then
    if jq empty <<< "$result" 2>/dev/null; then
      api_build_response::retrodeck-api "success" "$request_id" "$result" ""
    else
      local json_result
      json_result=$(jq -c -n --arg r "$result" '$r')
      api_build_response::retrodeck-api "success" "$request_id" "$json_result" ""
    fi
  else
    ...
    api_build_response::retrodeck-api "error" "$request_id" "" "$error_msg"
  fi

  exec 3>&-
}

api_send_progress::retrodeck-api() {
  # USAGE: api_send_progress::retrodeck-api "$current" "$total" "$message"
...
api_build_response::retrodeck-api() {
  # USAGE: api_build_response::retrodeck-api "$status" "$request_id" "[$result_json]" "[$error_message]"
...
api_builtin_list_endpoints::retrodeck-api() {
  # USAGE: api_builtin_list_endpoints::retrodeck-api
...
api_check_duplicate_endpoints::retrodeck-api() {
  # USAGE: api_check_duplicate_endpoints::retrodeck-api
