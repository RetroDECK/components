...
downloader_info() {
  echo "type:codeberg_release,codeberg-release"
}

is_codeberg_release_url() {
  local url="$1"
  [[ "$url" =~ codeberg\.org/[^/]+/[^/]+/releases ]]
}
...
download() {
  ...
  if is_codeberg_release_url "$url"; then # Validate URL is for a Codeberg release
    log info "Processing Codeberg release URL"

    # Parse Codeberg URL to get owner/repo
    local owner repo
    read -r owner repo <<< "$(parse_codeberg_url "$url")"

    if [[ ! -n "$owner" || ! -n "$repo" ]]; then
      log error "Could not parse Codeberg owner/repo from URL: $url"
      return 1
    fi

    if [[ "$version" == "latest" ]]; then # Resolve "latest" version if needed
      log info "Resolving $version version for $owner/$repo"
      resolved_version=$(get_latest_codeberg_release_version "$owner" "$repo")
      ...
      log info "Resolving $version version for $owner/$repo"
      resolved_version=$(get_newest_codeberg_release_version "$owner" "$repo")
      if [[ $? -ne 0 || -z "$resolved_version" ]]; then
    ...
    if [[ "$asset_name" == *"*"* ]]; then
      ...
      final_url=$(get_codeberg_release_asset_url "$owner" "$repo" "$resolved_version" "$asset_name")
    ...
    log error "Provided URL is not for a Codeberg release"
    return 1
  fi
  ...
  download_cmd() {
    curl --silent --location --output "$final_dest" "$final_url" 2>&1
  }
