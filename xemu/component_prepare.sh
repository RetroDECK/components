...
if [[ "$action" == "reset" ]]; then # Run reset-only commands
    ...
    if [[ $multi_user_mode == "true" ]]; then # Multi-user actions
        ...
        cp -fv "$config/xemu.toml" "$xemu_conf"
        set_setting_value "$xemu_conf" "screenshot_dir" "'$rd_home_screenshots_path'" "xemu" "General"
        set_setting_value "$xemu_conf" "bootrom_path" "'$rd_home_bios_path/mcpx_1.0.bin'" "xemu" "sys.files"
        set_setting_value "$xemu_conf" "flashrom_path" "'$rd_home_bios_path/Complex.bin'" "xemu" "sys.files"
        set_setting_value "$xemu_conf" "eeprom_path" "'$rd_home_saves_path/xbox/xemu/xbox-eeprom.bin'" "xemu" "sys.files"
        set_setting_value "$xemu_conf" "hdd_path" "'$rd_home_bios_path/xbox_hdd.qcow2'" "xemu" "sys.files"
    fi
