...
if [[ "$action" == "reset" ]]; then # Run reset-only commands
    ...
    if [[ $multi_user_mode == "true" ]]; then # Multi-user actions
        ...
        sed -i 's^\^$(EmulatorDir): .*^$(EmulatorDir): '"$rd_home_bios_path/rpcs3/"'^' "$multi_user_data_folder/$SteamAppUser/config/rpcs3/vfs.yml"
        ...
        # This is an unfortunate one-off because set_setting_value does not currently support settings with $ in the name.
        sed -i 's^\^$(EmulatorDir): .*^$(EmulatorDir): '"$rd_home_bios_path/rpcs3/"'^' "$rpcs3vfsconf"
        set_setting_value "$rpcs3vfsconf" "/games/" "$rd_home_roms_path/ps3/" "rpcs3"
        dir_prep "$rd_home_saves_path/ps3/rpcs3" "$rd_home_bios_path/rpcs3/dev_hdd0/home/00000001/savedata"
        dir_prep "$rd_home_states_path/ps3/rpcs3" "$XDG_CONFIG_HOME/rpcs3/savestates"
    fi

    # Shared actions
    create_dir "$rd_home_bios_path/rpcs3/dev_hdd0"
    create_dir "$rd_home_bios_path/rpcs3/dev_hdd1"
    create_dir "$rd_home_bios_path/rpcs3/dev_flash"
    create_dir "$rd_home_bios_path/rpcs3/dev_flash2"
    create_dir "$rd_home_bios_path/rpcs3/dev_flash3"
    create_dir "$rd_home_bios_path/rpcs3/dev_bdvd"
    create_dir "$rd_home_bios_path/rpcs3/dev_usb000"
fi
