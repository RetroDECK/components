...
if [[ "$action" == "reset" ]]; then # Run reset-only commands
    ...
    if [[ $multi_user_mode == "true" ]]; then # Multi-user actions
        ...
        set_setting_value "$dolphinconf" "ISOPath0" "$rd_home_roms_path/wii" "dolphin" "General"
        set_setting_value "$dolphinconf" "ISOPath1" "$rd_home_roms_path/gc" "dolphin" "General"
        set_setting_value "$dolphinconf" "WiiSDCardPath" "$rd_home_saves_path/wii/dolphin/sd.raw" "dolphin" "General"
    fi
