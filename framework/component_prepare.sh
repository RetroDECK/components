...
if [[ "$action" == "reset" ]]; then # Update the paths of all folders in retrodeck.cfg and create them
    while read -r config_line; do
        ...
        if [[ ! $current_setting_name =~ (rd_home_path|sdcard) ]]; then # Ignore these locations
            ...
            if [[ ! $current_setting_name == "rd_internal_logs_path" ]]; then # Don't create a logs folder normally, this will be a symlink to the internal logs folder in /var/config/retrodeck/logs
