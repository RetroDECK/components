...
if [[ "$action" == "reset" ]]; then # Run reset-only commands
    ...
    if [[ $multi_user_mode == "true" ]]; then # Multi-user actions
        ...
        set_setting_value "$primehack_config_Dolphin" "ISOPath0" "$rd_home_roms_path/wii" "primehack" "General"
        set_setting_value "$primehack_config_Dolphin" "ISOPath1" "$rd_home_roms_path/gc" "primehack" "General"
    fi
...
if [[ "$action" == "postmove" ]]; then # Run only post-move commands
    ...
    dir_prep "$rd_home_mods_path/Primehack" "$XDG_DATA_HOME/primehack/Load/GraphicMods"
    dir_prep "$rd_home_texture_packs_path/Primehack" "$XDG_DATA_HOME/primehack/Load/Textures"
    set_setting_value "$primehack_config_Dolphin" "ISOPath0" "$rd_home_roms_path/gc" "primehack" "General"
fi
